...
class XEGNNK(MessagePassing):
    ...
    def __init__(self, invariant_node_feat_dim=64, invariant_edge_feat_dim=32,
            n_vector_features=128):
        super().__init__(node_dim=0, aggr=None,
                         flow="source_to_target")  # ! This should be target to source
        ...
        self.phi_message = MLP(self.message_input_size, invariant_node_feat_dim,
        ...
        if self.use_cross_product:
            self.phi_x_cross = MLP(invariant_node_feat_dim, invariant_node_feat_dim,
                                   n_vector_features)
...
class DiTeBlock(nn.Module):
    ...
    def __init__(
        ...
        self.feature_embedder = MLP(
            hidden_size + hidden_size + invariant_edge_feat_dim + dist_size, hidden_size,
            hidden_size
        )
        ...
        self.lin_edge1 = nn.Linear(
            invariant_edge_feat_dim + n_vector_features * scale_dist_features,
            invariant_edge_feat_dim, bias=False
        )
...
class Megalodon(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            self.dit_layers.append(
                DiTeBlock(
                    invariant_node_feat_dim, num_heads, use_z=False,
    ...
    def forward(self, batch, X, H, E_idx, E, t):
        ...
        for layer_index in range(len(self.dit_layers)):
            ...
            pos = self.egnn_layers[layer_index](batch, pos, H, E_idx, edge_attr,
                                                te_e)  # ! TODO at time here
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            self.egnn_layers.append(
                XEGNNK(invariant_node_feat_dim, invariant_edge_feat_dim, n_vector_features))
        # self.h_feat_refine = DiTBlock(invariant_node_feat_dim, num_heads, use_z=False)
        self.node_blocks = nn.ModuleList(
            [nn.Linear(invariant_node_feat_dim, invariant_node_feat_dim) for i in range(num_layers)]
        )
...
class SkinnyDiTeBlock(nn.Module):
    ...
    def __init__(
        ...
        self.adaLN_modulation = nn.Sequential(nn.SiLU(),
                                              nn.Linear(hidden_size, 6 * hidden_size, bias=True))
        self.adaLN_edge_modulation = nn.Sequential(nn.SiLU(),
        ...
        # self.lin_edge1 = nn.Linear(hidden_size, hidden_size, bias=False)
        self.lin_edge1 = nn.Linear(hidden_size + n_vector_features * scale_dist_features,
...
class EGNNK(nn.Module):
    ...
    def __init__(self, invariant_node_feat_dim=64, n_vector_features=128, scale_dist_feats=4):
        ...
        self.use_cross_product = True
        if self.use_cross_product:
            self.phi_x_cross = MLP(self.message_input_size, invariant_node_feat_dim,
                                   invariant_node_feat_dim)
        self.x_norm = E3Norm(n_vector_features)

        self.phi_h = MLP(
            invariant_node_feat_dim + invariant_node_feat_dim, invariant_node_feat_dim,
            invariant_node_feat_dim
        )
        self.phi_e = MLP(invariant_node_feat_dim, invariant_node_feat_dim, invariant_node_feat_dim)
        # TODO: @Ali what is good weight inititalization for EGNN?

    def forward(self, batch, X, H, edge_index, edge_attr=None, te=None, edge_batch=None,
            distances=None):
        X = X - scatter_mean(X, index=batch, dim=0, dim_size=X.shape[0])[batch]
        X = self.x_norm(X, batch)
        H = self.h_norm(H, batch)
...
class MegalodonDotFNNoResSkinny(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            self.egnn_layers.append(
                EGNNK(invariant_node_feat_dim, scale_dist_feats=self.scale_dist_features))
        # self.h_feat_refine = DiTBlock(invariant_node_feat_dim, num_heads, use_z=False)
        # self.node_blocks = nn.ModuleList(
        #     [nn.Linear(invariant_node_feat_dim, invariant_node_feat_dim) for i in range(num_layers)]
        # )
...
class PygDiTeBlock(nn.Module):
    ...
    def __init__(
        ...
        if scale_dist_features > 1:
            ...
            dist_size = n_vector_features
        self.feature_embedder = MLP(hidden_size + hidden_size + hidden_size + dist_size,
