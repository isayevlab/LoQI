...
class PredictionHead(nn.Module):
    def __init__(self, num_classes, feat_dim, discrete=True, edge_prediction=False,
    ...
    def forward(self, batch, H):
        ...
        if self.discrete:
            probs = F.softmax(logits,
                              dim=-1)  # scatter_softmax(logits, index=batch, dim=0, dim_size=H.size(0))
        else:
